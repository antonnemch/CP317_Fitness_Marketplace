...
import { logoutUser, getUserData, getToken } from './api/auth-api';
...
const AuthProvider = ({ children }) => {
    ...
    const auth = { user, isAuthenticated, login, logout, role: user?.role || 'guest' };
...
// --- 2. Main App Component with Global State ---
const App = () => {
    // Note: The App component now acts as the layout container.
    return (
...
// Helper component for layout and context access
const AppLayout = () => {
    const auth = useAuth();
    
    // Simple top bar navigation (not the full AppRouter, just the header)
    return (
            ...
            <header className="global-header">
                ...
                <div className="nav-info">
                    {auth.isAuthenticated ? (
                        <>
                            <span>Welcome, **{auth.user.email}** ({auth.role})</span>
                            <button onClick={auth.logout} className="primary-btn">Logout</button>
                        </>
                    ) : (
                        <span><Link to="/login">Login</Link> / <Link to="/signup">Register</Link></span>
                    )}
                </div>
            </header>
            
            {/* Renders the actual page components */}
...
// Export the main component (used in main.jsx)
export default App;